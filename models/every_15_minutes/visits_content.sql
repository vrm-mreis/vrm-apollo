...
FROM (
  SELECT
    ...
    coalesce(visits_mod.article_tags,
      content.max_article_tags) AS article_tags,
    Z_host,
    Z_url,
    coalesce(visits_mod.publishtime,
      content.min_publishtime) AS Z_publishtime,
    coalesce(visits_mod.vrm_premiumarticle,
      content.min_vrm_premiumarticle) AS Z_vrm_premiumarticle
  FROM
    visits_mod
  LEFT JOIN (
    SELECT
      DISTINCT site_id,
      article_id,
      MAX(vrm_article_tags) AS max_article_tags,
      MIN(publishtime) AS min_publishtime,
      MIN(vrm_premiumarticle) AS min_vrm_premiumarticle
    FROM
      {{ source('piano', 'content_profile') }}
    GROUP BY
      site_id,
      article_id) AS content
  ON
